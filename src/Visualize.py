...
def load_model_from_checkpoint(ckpt_path, H, W, model_type="auto"):
    ...
    if model_type == "enhanced":
        model = EnhancedQNet(H, W, use_attention=True, use_residual=True, use_multiscale=True)
        print("Using Enhanced QNet")
    elif model_type == "enhanced_memory":
        model = EnhancedQNet(H, W, use_attention=False, use_residual=True, use_multiscale=False)
...
def render_static_firebreak_plot(env, placed_indices, raster_for_bg, out_path=None):
    ...
    if placed_indices:
        ...
        sc = ax1.scatter(xs, ys, c=order, s=30, cmap="cool", 
...
def render_frame(env, step_mask, step_no, obs):
    ...
    bg = obs[0] if obs.ndim == 3 else obs[0, :, :]
    
    fig, ax = plt.subplots(figsize=(6, 6))
    ax.imshow(bg, cmap="hot", aspect='equal')
    
    # Show fuel breaks placed in this step
    if np.any(step_mask):
        ys, xs = np.where(step_mask)
        ax.scatter(xs, ys, s=50, c='cyan', marker='s', 
...
def render_q_value_heatmap(q_maps, placed_indices, out_path=None):
    ...
    steps_to_show = [0, len(q_maps)//4, len(q_maps)//2, 3*len(q_maps)//4, len(q_maps)-1]
...
def parse_args():
    ...
    p.add_argument("--sims", type=int, default=5, help="num_simulations (reduced for viz)")
    p.add_argument(
...
def main():
    ...
    print(f"Average reward per step: {result['total_reward']/max(1, result['steps']):.3f}")
    if result["burned_areas"]:
